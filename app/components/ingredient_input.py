...
from utils.helpers import _current_input_key, _get_current_text  # keep imports in case used elsewhere
...
def render_ingredient_input():
    ...
    <div class="section-title">
        <h3 style="font-size: 1.45rem;">Type your individual ingredients</h3>
    </div>
    """,
    unsafe_allow_html=True,
)

    input_key = _current_input_key()

    # Use a form so Enter and the button do the same thing
    with st.form("ingredient_text_form", clear_on_submit=True):
        ...
        with col2:
            # KEEP the same label, type, and use_container_width for styling
    ...
    if st.session_state.get("ingredient_warning"):
        st.markdown(
            ...
            """.format(st.session_state.ingredient_warning),
            unsafe_allow_html=True,
        )
        # Clear the warning after displaying it
        st.session_state.ingredient_warning = None

    # ---------------- INGREDIENT LIST (unchanged from your old code) ----------------
    if st.session_state.ingredients:
        ...
        st.markdown(
            ...
            """.format(len(st.session_state.ingredients)),
            unsafe_allow_html=True,
        )
