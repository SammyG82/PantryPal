...
# # Make the project root (PantryPal/) importable so "scripts" is visible
# PROJECT_ROOT = os.path.abspath(os.path.join(os.path.dirname(__file__), "..", ".."))
# if PROJECT_ROOT not in sys.path:
#     sys.path.append(PROJECT_ROOT)

# from scripts.recipe_search import load_recipes, match_recipes
...
# # --- NEW: simple dataset-backed matching ---
# from pathlib import Path


# @st.cache_data(show_spinner=False)
# def _load_df():
#     # your file is now in data/raw/recipes.csv
...
# if ings:
#     matches = match_recipes(ings, df, top_k=3)

#     if not matches:
#         st.info("No direct matches found. Try adding more common ingredients ✨")
#     else:
#         for i, (name, hits, total) in enumerate(matches, start=1):
#             st.markdown(f"""
#             <div class="recipe-card">
#               <h3>{i}. {name}
#                 <span class="health-badge badge-balanced">Match: {hits} ing(s)</span>
#               </h3>
#               <p><b>Matched your ingredients:</b> {hits} • <b>Recipe size:</b> {total}</p>
#               <p class="muted">Showing first 3 matches (simple OR-match).</p>
#             </div>
#             """, unsafe_allow_html=True)
# else:
#     st.info("Type a few ingredients on the Home page to see dataset matches here.")
